...
namespace ExpensePlus
{
    public partial class Investments : Form
    {
        ...
        private void BuildInvestmentDropDown()
        {
            ...
            cmbChartInvestments.DataSource = chartData.Tables[0].AsEnumerable().Select(x => x.Field<string>("InvestName")).Distinct().ToList();
        ...
        {
            var filteredXValsData = chartData.Tables[0].AsEnumerable().Where(x => x.Field<string>("InvestName") == InvestName).Select(x => x.Field<int>("InvestmentProjectionYear")).ToArray();
            var filteredYValsData = chartData.Tables[0].AsEnumerable().Where(x => x.Field<string>("InvestName") == InvestName).Select(x => x.Field<decimal>("ProjectionValue")).ToArray();
        ...
        private void addInvestmentToolStripMenuItem_Click(object sender, EventArgs e)
        {
